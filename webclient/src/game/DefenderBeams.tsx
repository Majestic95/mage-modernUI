...
/**
 * Single-beam core opacity. Live-test verdict round 2 (2026-05-09):
 * doubled from 0.234 to 0.468 because the tighter cone aperture
 * spreads less area; at the alpha-reduced glow tokens (effective
 * per-beam alpha ~0.23) the directional spotlight now actually
 * reads as a colored beam instead of a faint smudge. Three-beam
 * worst-case stacking at the viewport apex hits ~1.4 nominal but
 * CSS clamps to 1.0 — the brightest pixel sits at the table center
 * which is already a focal-card surface, so the visual reads as
 * "the lights are on for this combat" rather than washing out.
 */
const BEAM_OPACITY = 0.468;

/**
 * Half-angle of the cone clip-path in radians. Live-test verdict
 * round 2 (2026-05-09) cut from π/12 (15° each side, 30° total) to
 * π/24 (7.5° each side, 15° total) — the prior cone still read as
 * floodlight; the user wanted a true spotlight aiming at the pod.
 * π/24 reads as a sharp directional pointer.
 */
const BEAM_HALF_ANGLE_RAD = Math.PI / 24;
