...
> - **Foundation: central-area mutex resolution** — RATIFIED + SHIPPED 2026-05-10 under Option D (z-layer cohabitation). [`docs/design/foundation-central-area-mutex.md`](foundation-central-area-mutex.md). Critic-pass log row: foundation slice in [`docs/decisions/critic-pass-log.md`](../decisions/critic-pass-log.md). Bundles 5 + 6 unblocked.
> - **Bundle 5 — Damage Moment:** scope brief landed 2026-05-10; ready to slice. [`docs/design/combat-bundle-5-damage-moment.md`](combat-bundle-5-damage-moment.md). 6 slices (5-F foundation + 5-A through 5-E + 5-X.0 critic pass) — includes a schema bump (1.34 → 1.35) at 5-F to add per-opponent commander damage to the wire so the lethal-21 cinematic ships from this branch.
> - **Bundles 2, 6:** still in brainstorm state; no per-bundle scope brief written yet. Bundle 6 (Arrow Storytelling) consumes Bundle 5's path-traversal infrastructure. Bundle 2 (Combat Stage) is independent but lower-leverage.
>
