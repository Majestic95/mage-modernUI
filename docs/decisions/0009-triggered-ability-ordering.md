...
### D5. `TRIGGER_AUTO_ORDER_*` — second `playerAction.data` shape

The existing `playerAction` envelope (ADR 0007 D6) carries `data: null` for most actions; for `ROLLBACK_TURNS` it's `{ "turns": <int> }`; for `REQUEST_AUTO_ANSWER_TEXT_*` it's `{ "text": "..." }`. Trigger auto-order needs two new data shapes:
...
- `TRIGGER_AUTO_ORDER_NAME_FIRST` / `_NAME_LAST` — `data: { "abilityId": "<uuid>" }`. **Not `ruleText`** — the wire carries only the ability id; the **facade** resolves the source name and the substituted rule text server-side, then forwards the substituted string to `MageServerImpl.sendPlayerAction(action, gameId, session, ruleText)`. This sidesteps a hard runtime exception: `HumanPlayer.setTriggerAutoOrder` at `:2843-2845` throws `IllegalArgumentException` if the rule text contains `{this}`, and `AbilityView.rules` carries the un-substituted form (`AbilityView.java:25` calls `ability.getRule()` with no source-name argument). Swing handles this at `GamePanel.java:3074-3076` by substituting the ability's source name client-side before sending; we keep substitution **server-side** because (a) the facade already has the game state to look up `ability.getSourceId()` → source name, (b) the webclient does not currently surface `AbilityView.sourceName` on the wire (F3), and (c) keeping the wire payload as just the ability id avoids two-mode plumbing in `WebPlayerAction.data`. Implementation: in the inbound handler (`GameStreamHandler.handlePlayerAction`), for `TRIGGER_AUTO_ORDER_NAME_*` actions, look up the pending trigger in `game.getState().getTriggered(playerId)`, compute `ability.getRule(sourceObject.getName())` (the same overload `HumanPlayer.java:1475` uses for the `triggerAutoOrderNameFirst` comparison), and pass the resolved string to `MageServerImpl.sendPlayerAction`. Without this substitution, the first user to click "always first by rule text" on any permanent-sourced trigger (most triggers — "When {this} enters the battlefield, ...") would crash the engine.
- `TRIGGER_AUTO_ORDER_RESET_ALL` — `data: null` (existing default).
...
After firing a `_LAST` action, the Swing client sends `sendPlayerUUID(gameId, null)` (`:3085`, `:3096`) "to refresh the displayed abilities". Mirroring this on the wire is not possible: `playerResponse{kind:"uuid"}` requires a JSON string value (`GameStreamHandler.java:418-426` validates `valueNode.isTextual()` then `UUID.fromString(...)` — a JSON `null` would fail the textual check and return `BAD_REQUEST`; an empty string would fail `UUID.fromString` and return `BAD_REQUEST`). The webclient instead closes the dialog and waits for the engine to re-fire `gameTarget` with the remaining abilities. **Uncertain — needs verification on a 3-trigger setup** that the engine does re-fire after a `_LAST` action without the explicit null-UUID nudge. (Reading `chooseTriggeredAbility`: the `while (canRespond())` loop re-iterates, so the next iteration should see the `_LAST`-deferred ability moved into `abilitiesOrderLast` and either fall through to the single-element return at `:1521-1524` or fire a new `gameTarget`. Confirm with a fixture in slice 27.)

`TRIGGER_AUTO_ORDER_RESET_ALL` is fire-and-forget; no follow-up dispatch.
...
### D11. Schema bump 1.15 → 1.16

Additive minor:

- `WebClientMessageOptions` gains `isTriggerOrder: boolean` (D2).
- `WebPlayerAction.data` permits `{ abilityId: string }` for **all four** `TRIGGER_AUTO_ORDER_ABILITY_*` and `TRIGGER_AUTO_ORDER_NAME_*` cases (D5). Single shape — the facade discriminates between "use this UUID directly" (`_ABILITY_*`) and "look up the rule and substitute `{this}`" (`_NAME_*`) by action enum.

`docs/schema/CHANGELOG.md` 1.16 entry. `SchemaVersion.CURRENT` constant updates.
...
- `playerAction` `data` shapes — single `{ abilityId: string }` for all four `TRIGGER_AUTO_ORDER_*_FIRST` / `_LAST` actions (D5). Server-side validation per-action.
- **Facade-side `{this}` substitution** for `_NAME_*` actions (D5 / E2). The inbound handler resolves `ability.getRule(sourceObject.getName())` and forwards the substituted string to `MageServerImpl.sendPlayerAction`. Without this the engine throws `IllegalArgumentException` at `HumanPlayer.java:2843-2845`.
- Two-step dispatch: action then `playerResponse{uuid}` for the `_FIRST` cases (D5).
- Test for the dispatch sequence; test for the `{this}`-substitution path with a fixture trigger whose rule contains `{this}`.
- Verify `_LAST` action follow-up — the engine re-fires `gameTarget` with the remaining abilities without an explicit nudge (OQ #3). If not, add the facade-side workaround.

### Slice 28 — Polish

- `WebCardView.sourceLabel` (or `WebAbilityView.sourceName`) extension — purely UX, for showing "from: <source>" attribution in each row. Slice 27 doesn't depend on this because `{this}` substitution lives server-side (D5 / E2), so the wire-side `sourceName` is purely cosmetic. Decide based on slice 26 testing whether the rule-text-only attribution feels too sparse, especially for emblem / dungeon / plane sources where `AbilityView.setName(...)` already overrides the name field (`CardsView.java:111-128`).
- "Reset all" button at the bottom of the panel (D8 menu's fifth option also lives at bottom-of-panel for discoverability).
...
- **`GameStreamHandler` inbound dispatch test** — `playerAction { action: "TRIGGER_AUTO_ORDER_ABILITY_FIRST", data: { abilityId: "<uuid>" } }` reaches `MageServerImpl.sendPlayerAction(TRIGGER_AUTO_ORDER_ABILITY_FIRST, gameId, sessionId, abilityUuid)` with the runtime UUID as the fourth arg. For `_NAME_FIRST` with the same `{ abilityId: "<uuid>" }` payload, the handler looks up the pending trigger, computes `ability.getRule(sourceObject.getName())` (substituting `{this}`), and passes the resolved **string** to `MageServerImpl.sendPlayerAction`. Test fixture: a fixture trigger whose raw rule text contains `{this}` (e.g. "When {this} enters the battlefield, ...") — assert the substituted string reaches `setTriggerAutoOrder` and **does not** trip the `IllegalArgumentException` guard at `HumanPlayer.java:2843-2845`. Negative: malformed `data` (wrong shape) → `streamError BAD_REQUEST`. Negative: `abilityId` does not match any pending trigger → `streamError BAD_REQUEST`.
...
- **`GameDialog` render test** — pending-dialog with `gameTarget + isTriggerOrder` renders the new panel (`data-testid="trigger-order-dialog"`) with one row per ability, each row's text containing the rule string. Old `data-testid="game-dialog"` for `gameTarget` continues to render the regular `TargetDialog` when `isTriggerOrder = false`.
...
3. **`_LAST` action follow-up:** Swing fires `sendPlayerUUID(gameId, null)` after `_LAST` (`GamePanel.java:3085`, `:3096`) "to refresh the displayed abilities" — comment is a TODO from 2021 doubting it's still needed. The webclient cannot mirror this on the wire — `playerResponse{kind:"uuid"}` is type-validated for a JSON string value at `GameStreamHandler.java:418-426`, and a JSON `null` would fail the `isTextual()` guard with `BAD_REQUEST`. **Verify** that the engine re-fires `gameTarget` with the remaining abilities **without** the explicit nudge (likely yes — the `while (canRespond())` loop at `HumanPlayer.java:1466` re-iterates and moves `_LAST`-tagged abilities to `abilitiesOrderLast`), and if not, options are: (a) facade-side detection of `_LAST` actions that synthesizes the no-op nudge upstream via `MageServerImpl.sendPlayerUUID(gameId, session, null)`; (b) closing the dialog client-side and trusting the engine's loop. Decide in slice 27 fixture testing.
