...
- **(e) Simultaneous-action prompts** (e.g., 2HG declare-attackers with shared turn): if one player disconnects mid-prompt and the other has already submitted, server holds for full reconnect-timeout (60s default), then auto-passes the disconnected player and emits `dialogClear`. **Shipped across slices 70-H + 70-H.5:** Slice 70-H (schema 1.23) lands the wire-field detection surface — `WebPlayerView.connectionState` populated server-side via `AuthService.connectionStateFor(gameId, playerId)` over a route-filtered socket count (per slice-70-H critic C3), plus the `WebDialogClear.REASON_TIMEOUT` constant. The PlayerFrame DISCONNECTED overlay (top-right corner pill, "Disconnected — waiting for reconnect", 0.7 opacity / 0.6 grayscale, per design-system §7.3) consumes the field. Slice 70-H.5 lands the per-prompt disconnect-timer state machine in `WebSocketCallbackHandler` (60s default, configurable via `XMAGE_DISCONNECT_TIMEOUT_SEC`, clamped to [30, 180]; arms on prompt-open + last-socket-close, cancels on reconnect or prompt-close frame), the cross-handler `dialogClear{reason:"TIMEOUT"}` broadcast via `AuthService.broadcastDialogClearToGame`, best-effort auto-pass dispatch by prompt method (`gameAsk → false`, `gameTarget → null`, etc.) via `MageServerImpl.sendPlayerXxx`, an `aria-live="polite"` connection-state announcer in `GameTable` (announces "alice disconnected" / "alice reconnected" once per transition for SR parity per critic UX-I3), and cleanup hooks on `closeAllSockets` / game-close frames (per critics C1, C2). The new `xmage_disconnect_timeouts_total` metric counts every fired timer.
