...
- **(a) Battlefield filter:** `webclient/src/game/Battlefield.tsx:212-225` opponent loop adds `&& !p.hasLeft` predicate. Eliminated player's PlayerArea collapses to a thin "ELIMINATED" overlay stub: name + status + remaining chat history. No active glow, no priority pill. **Amended by ADR 0011 D2 (slice 70-D):** the layout-collapse behavior is now feature-flagged behind `VITE_FEATURE_KEEP_ELIMINATED`. Default `false` keeps the legacy collapse described above; slice 70-E flips the flag to `true` to ship the new "keep-with-slash overlay" treatment (eliminated seats stay in the layout with a diagonal red claw-rip slash + permanent fade-out). The earlier slice-69b commit's claim that "layout-collapse is unambiguous and matches MTGA precedent" is **superseded** — MTGA actually uses the keep-with-slash treatment matched in 70-E. Until 70-E flips the flag, this section's collapse semantics are the runtime behavior; after 70-E, the keep-with-slash semantics are.
- **(b) Dialog reconciliation:** any open dialog (vote loop, target prompt, cost decision) targeting the leaver dismisses via a new `dialogClear{playerId, reason: 'PLAYER_LEFT'}` wire frame. Vote loops in `VoteHandler.doVotes` skip the leaver server-side (engine already does, per Reviewer A1 evidence at `VoteHandler.java:33-39`); the new frame announces this to all clients so stuck modals close. If the engine then re-prompts a different player after skip (e.g., choose-new-target), that arrives as a fresh `gameAsk` envelope — the client does NOT chain off `dialogClear`. `dialogClear` is a fire-and-forget UI-teardown signal, not a transition trigger. **Reconnect ordering caveat (slice 69c):** the synthesized `dialogClear` frame reuses the triggering `gameUpdate`'s `messageId` so the two frames sit adjacent in the per-handler resume buffer. A reconnect with `?since=N` where N equals the leaver-transition `gameUpdate`'s messageId replays only frames with `messageId > N` — the dialogClear is skipped. This is acceptable because the gameUpdate at messageId=N+1 (or the carried-forward GameView in any later frame) already encodes `hasLeft=true` for the leaver, from which the client can infer the teardown. The contract is "**at most one** dialogClear emitted per leaver per game," not "exactly one delivered to every client over every reconnect path."
  ...
  **v2 scope of "targeting the leaver" (slice 69d).** The client-side dismissal logic in `webclient/src/game/store.ts` checks `pendingDialog.data.targets.includes(leaverId)`. This covers the canonical cases — `gameTarget` / `gameSelectAmount` / `gameAsk` with explicit target lists — but two structural cases are deferred to the slice 73-77 polish micro-batch:
  - **`gameAsk` with leaver-named-in-message-text** (e.g. "Allow alice to copy this spell?"): `targets` is empty; only the `message` string references the leaver. Catching this needs the leaver's *username* on the wire (currently only `playerId` ships), plus a substring scan that's locale-fragile. v2 leaves these dialogs visible — the user can dismiss manually; the engine's skip-leaver server-side logic still advances the game correctly.
  - **`gameSelect` with controller-of-target leaving** (declare-attackers / declare-blockers): `targets` carries permanent UUIDs, never player UUIDs, so a leaver match is structurally impossible. When alice's controller leaves mid-combat, alice's permanents either leave the battlefield (state-based actions) or change controller; the next `gameUpdate` reflects this and the prompt's relevance is implicitly resolved. v2 leaves these dialogs visible until the gameUpdate reshapes the available targets.
  
  Both gaps are user-survivable (engine progresses correctly; only the visual cue is delayed). Slice 73-77 may add a `dialogClear` reason variant or a server-side dialog-context inspection if the gap proves disruptive in playtesting.
...
- **(e) Simultaneous-action prompts** (e.g., 2HG declare-attackers with shared turn): if one player disconnects mid-prompt and the other has already submitted, server holds for full reconnect-timeout (60s default), then auto-passes the disconnected player and emits `dialogClear`. **Partially shipped slice 70-H (schema 1.23):** the wire-field detection surface lands — `WebPlayerView.connectionState` (values `"connected"` / `"disconnected"`, populated server-side via `AuthService.connectionStateFor(gameId, playerId)` over a route-filtered socket count per slice-70-H critic C3) plus the `WebDialogClear.REASON_TIMEOUT` constant for forward-compat. The PlayerFrame DISCONNECTED overlay (top-right corner pill + 0.7 opacity / 0.6 grayscale frame desaturation, lighter than eliminated per design-system §7.3) consumes the field. The pill copy reads "Disconnected — waiting for reconnect" so users understand the wait is intentional rather than a hung client (per slice-70-H critic UX-C1). **Deferred to slice 70-H.5:** the per-prompt timer state machine, the cross-handler `dialogClear{reason:"TIMEOUT"}` broadcast via `AuthService.broadcastToGame`, the auto-pass response to the engine, AND an `aria-live="polite"` region announcing connection-state transitions to SR users in real time (per slice-70-H critic UX-I3 — the current implementation surfaces "disconnected" via the role="group" aria-label composition, which most SR engines announce only on focus-enter rather than on attribute change). Detection without auto-pass is a UX-improvement-only intermediate (other players see "Disconnected — waiting for reconnect" instead of "waiting on Bob forever"); the auto-pass restores game progress and ships in 70-H.5 alongside the timer.
