...
|---|---|---|
| Language | **Java 17+ LTS — single toolchain** | JDK 17 minimum, JDK 21 fine. Records, sealed types, pattern matching, `var`, text blocks all available in 17. JDK 17+ compiles upstream's Java 8 modules via `--release 8`. **Phase 1 spike (2026-04-25) confirmed `--add-opens` flags let JBoss Remoting 2.5.4 work on JDK 17+ for both server and client launches** — see [ADR 0003](0003-embedding-feasibility.md). Single JDK toolchain end-to-end; no JDK 8 install required. The `--add-opens` bundle is documented in `Mage.Server.WebApi/pom.xml` and `docs/dev-setup.md`. |
